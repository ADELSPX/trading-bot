#!/usr/bin/env python3
"""Debug: show all zones sorted by proximity"""
...
print(f"SPX={px:.0f} | Zones={len(sd.zones)}")
for z in sorted(sd.zones, key=lambda z: abs(z.mid - px)):
    dist = abs(px - z.mid) / px * 100
    print(f"  {z.zone_type.value:6s} | {z.state.value:8s} | mid={z.mid:.0f} | dist={dist:.1f}% | candles={z.num_candles_inside}")
